...
const Vista2 = ({ navigation }) => {
  ...
  return (
    ...
    // Calcula las diferencias específicas para cada tarjeta
    let diffPromedio = '0.00';
    let diffParalelo = '0.00';
    if (index === 0 && data.length > 2) {
      const customAverage = ((parseFloat(data[0].promedio) + parseFloat(data[2].promedio)) / 2).toFixed(2);
      diffPromedio = (parseFloat(amount / parseFloat(data[0].promedio)) - parseFloat(amount / parseFloat(customAverage))).toFixed(2);
      diffParalelo = (parseFloat(amount / parseFloat(data[0].promedio)) - parseFloat(amount / parseFloat(data[2].promedio))).toFixed(2);
    } else if (index === 1 && data.length > 2) {
      const customAverage = ((parseFloat(data[0].promedio) + parseFloat(data[2].promedio)) / 2).toFixed(2);
      diffPromedio = (parseFloat(amount / parseFloat(customAverage)) - parseFloat(amount / parseFloat(data[0].promedio))).toFixed(2);
      diffParalelo = (parseFloat(amount / parseFloat(customAverage)) - parseFloat(amount / parseFloat(data[2].promedio))).toFixed(2);
    } else if (index === 2 && data.length > 2) {
      const customAverage = ((parseFloat(data[0].promedio) + parseFloat(data[2].promedio)) / 2).toFixed(2);
      diffPromedio = (parseFloat(amount / parseFloat(data[2].promedio)) - parseFloat(amount / parseFloat(data[0].promedio))).toFixed(2);
      diffParalelo = (parseFloat(amount / parseFloat(data[2].promedio)) - parseFloat(amount / parseFloat(customAverage))).toFixed(2);
    }
        ...
        <Text style={styles.cardTitle}>{prefix} - Tasa: {tasaConDosDecimales}</Text>
          ...
          <View style={styles.additionalInfo}>
            {data.length > 2 ? (
              <>
                ...
                <Text style={styles.infoText}>Promedio: {diffPromedio}</Text>
                <Text style={styles.infoText}>Paralelo: {diffParalelo}</Text>
              </>
            ) : (
